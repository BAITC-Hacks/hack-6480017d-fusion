...
2. Открыть #5 — `100000004015047100`: consolidator, приоритет **0,625090**, место **5 из 2248**. В «Почему такой приоритет» видны пять слагаемых; в раскрываемых денежных потоках — **9 плательщиков / 1 получатель**, **919 104 / 69 195 KZT**, полный ID контрагента и число переводов. Нажать ID, чтобы перейти к его карточке. Запасной пример транзита — `100000003635170100`: 3/4 контрагента, 268 500/253 500 KZT.
...
4. Выбрать кластер 0: **278 участников, 1 seed, 31 237 334,24 KZT** внутри группы. Раскрыть приоритетных участников, переключить окраску графа и скачать три CSV. Основное демо работает без LLM; OpenAI — необязательная справка со ссылками на факты.
...
Аудит полностью читает три parquet и выполняет **34 проверки** схем, ссылок, глубин, дат, изолятов и агрегатов каждой пары. Он обновляет [DATA_AUDIT.md](docs/DATA_AUDIT.md), не меняя исходные данные; провал проверки возвращает ненулевой код. На 23 сентября 2026 пройдены **83 backend-теста** и сборка frontend. Проверяются точность ID, изоляты, граница, правила, граф, CSV, полный ранг, потоки вне лимита графа, self-loop, ошибки данных, AI-схемы, ссылки, кеш, таймауты и отсутствие вызовов отключённой NVIDIA. Актуальный статус — [PROGRESS.md](docs/PROGRESS.md).
